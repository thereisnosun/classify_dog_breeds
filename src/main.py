...
def get_model():
    ...
    #opt = SGD(learning_rate=0.001, momentum=0., nesterov=True)
...
def load_images_by_hand():
    print('Loading train images from {0}...'.format(PREPROCESSED_IMAGES_FOLDER))
...
def run(model, x_train, y_train, x_validate, y_validate):
    history = model.fit(x=x_train, y=y_train, batch_size=BATCH_SIZE, epochs=EPOCH_NUMS,
...
def run_with_generators(model, use_from_source=True, save_to_dir=False,
    ...
    history = model.fit_generator(train_generator, epochs=EPOCH_NUMS,
            steps_per_epoch=761 // BATCH_SIZE, validation_steps=191 // BATCH_SIZE,
...
# x_train, y_train, x_validate, y_validate = load_images_by_hand()
# model = run(model, x_train, y_train, x_validate, y_validate)
